...
local function nolag(caller, targets)
	if not NADMOD then
		return
	end -- We should always have NADMOD installed, so the sanity check just needs to return
	...
	for _, prop in pairs(NADMOD.Props) do
		if
			prop.Owner ~= game.GetWorld()
			and (not targets or targetLut[prop.Owner])
			and IsValid(prop.Ent)
		then
	...
	ulx.fancyLogAdmin(
		caller,
		("#A froze %u prop%s owned by #T"):format(count, suffix),
		targets
	)
end
