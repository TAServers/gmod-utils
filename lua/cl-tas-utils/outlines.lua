-- High-performance outline implementation: https://github.com/Facepunch/garrysmod/pull/1590/
-- Modified to be enclosed in the TASUtils namespace and to fit the linter rules.
...
function outline.Add(ents, color, mode)
	if ListSize >= 255 then
		return
	end -- Maximum 255 reference values
	...
	if
		...
		and mode ~= OUTLINE_MODE_NOTVISIBLE
		and mode ~= OUTLINE_MODE_VISIBLE
	then
		mode = OUTLINE_MODE_BOTH
	end
...
function outline.SetRenderType(render_type)
	if
...
function outline.GetRenderType()
	return RenderType
end

function outline.SetDoubleThickness(thickness)
...
function outline.IsDoubleThickness()
...
local function RenderOutlines()
	hook.Run("SetupOutlines", outline.Add)
